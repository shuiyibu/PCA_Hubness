function [idx, C, sumD, D] = GHPKM(X, k, k_ocurrence, varargin)
...
dflts =  {'sqeuclidean' 'sample'          []       200        'error'  'notify'};
...
for rep = 1:reps
    ...
    converged = false;
    iter = 0;
    t = 30; %1. initialize temperature ---anneal
    theta=1;
    while true
        ...
        if iter > 0
            theta =min(1,iter/t); %getProbFromSchedule(t,iter); %2.   ---anneal
            totsumD = sum(D((idx-1)*n + (1:n)'));
            % Test for a cycle: if objective is not decreased, back out
            % the last step and move on to the single update phase
            if prevtotsumD <= totsumD
                idx = previdx;
                [C(changed,:), m(changed)] = gcentroids(X, idx, changed, distance, Xsort, Xord,theta,k_ocurrence);
                iter = iter - 1;
                break;
            end
            if display > 2 % 'iter'
                ...
                if ~isempty(empties)
                    ...
                    case 'singleton'
                        ...
                        for i = empties
                            ...
                            [C(from,:), m(from)] = gcentroids(X, idx, from, distance, Xsort, Xord,theta,k_ocurrence);
...
%------------------------------------------------------------------
%3.-------anneal
function [centroids, counts] = gcentroids(X, index, clusts, dist, Xsort, Xord,theta,k_ocurrence)
    ...
    if length(members) > 0
        ...
        switch dist
            case 'sqeuclidean'
                ...
                %4.-------anneal
                if rand <  theta
                    %DataPoint h = findClusterHub(members);
                    centroids(i,:) = sum(X(members,:),1) / counts(i);
                else
                    chooseProb=0;
                    for j=1:length(members)
                        chooseProb=chooseProb+k_ocurrence(members(j))^2;
                    end
                    sortk_ocurrence=zeros(2,length(members));
                    for j=1:length(members)
                        sortk_ocurrence(1,j)=k_ocurrence(members(j))^2;
                        sortk_ocurrence(2,j)=members(j);
                    end

                    [vsk,isk]=sort(sortk_ocurrence(1,:),2);

                    foundIndex=ceil(rand*chooseProb);

                    ci=sortk_ocurrence(2,isk(1));
                    for j=1:length(members)
                        if(vsk(j)<foundIndex)
                            ci=sortk_ocurrence(2,isk(j));
                        end
                    end

                    centroids(i,:) = X(members(ci),:);
                end
            case 'cityblock'
                % Separate out sorted coords for points in i'th cluster,
