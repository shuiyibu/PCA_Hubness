function [idx, C, sumD, D] = GHPKM(X, k, k_ocurrences, varargin)
...
for rep = 1:reps
    ...
    converged = false;
    iter = 0;
    t = 10; %1. initialize temperature ---anneal
    theta=1;
    while true
        ...
        if iter > 0
            ...
            if prevtotsumD <= totsumD
                idx = previdx;
                [C(changed,:), m(changed)] = gcentroids(X, idx, changed, distance, Xsort, Xord,theta,k_ocurrences);
                iter = iter - 1;
                break;
            end
            if display > 2 % 'iter'
            ...
            switch emptyact
                ...
                case 'singleton'
                    ...
                    for i = empties
                        ...
                        [C(from,:), m(from)] = gcentroids(X, idx, from, distance, Xsort, Xord,theta,k_ocurrences);
    ...
    while iter < maxit
        ...
        switch distance
            case 'sqeuclidean'
                ...
                C(nidx,:) = C(nidx,:) + (X(moved,:) - C(nidx,:)) / (max(m(nidx),1));
                C(oidx,:) = C(oidx,:) - (X(moved,:) - C(oidx,:)) / (max(m(oidx),1));
                C;
                err=find(C==Inf);
                if(length(err)>0)
                    ii=0;
...
%------------------------------------------------------------------
%3.-------anneal
function [centroids, counts] = gcentroids(X, index, clusts, dist, Xsort, Xord,theta,k_ocurrences)
...
%                 else
                    ...
                    for j=1:length(members)
                        chooseProb=chooseProb+k_ocurrences(members(j));
                    end
                    sortk_ocurrence=zeros(1,length(members));
                    for j=1:length(members)
                        sortk_ocurrence(j)=k_ocurrences(members(j));
                    end
                    
                    [vsk,isk]=sort(sortk_ocurrence,2);
